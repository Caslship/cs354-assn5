...
KdTree::KdTree(std::vector<Geometry *>& objects, int depth)
{
	_bounds = NULL;
	_objects = NULL;
	_left = NULL;
	...
	_bounds = new BoundingBox(objects[0]->getBoundingBox());
	for (int i = 1; i < num_objects; i++)
		_bounds->merge(objects[i]->getBoundingBox());

	// No point in subdividing further if we have at most 2 objects in a node
	if (num_objects < 3)
	{
	...
	// Have to figure out another way to end the recursions
...
bool KdTree::intersect(ray& r, isect& i) const
{
	...
	if (!_left && !_right && _objects)
	{
		bool have_one = false;
		typedef vector<Geometry*>::const_iterator iter;
		for(iter j = _objects->begin(); j != _objects->end(); ++j) {
			isect cur;
		...
		return (_left->intersect(r, i) || _right->intersect(r, i));
}
